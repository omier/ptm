...
public abstract class ConditionParser extends CommonCommand {
	public LinkedList<Command> cmds;
	...
	public void updateCondition(String par1) throws Exception {
		...
		if (par1.contains("==")) {
			exps = par1.split("==");
			this.condition = new EqualsExpression(sy.calc(exps[0], server), sy.calc(exps[1], server));
		} else if (par1.matches(".*<[^=].*")) {
			exps = par1.split("<");
			this.condition = new LessExpression(sy.calc(exps[0], server), sy.calc(exps[1], server));
		} else if (par1.contains("<=")) {
			exps = par1.split("<=");
			this.condition = new LessEqualsExpression(sy.calc(exps[0], server), sy.calc(exps[1], server));
		} else if (par1.matches(".*>[^=].*")) {
			exps = par1.split(">");
			this.condition = new GreaterExpression(sy.calc(exps[0], server), sy.calc(exps[1], server));
		} else if (par1.contains(">=")) {
			exps = par1.split(">=");
			this.condition = new GreaterEqualsExpression(sy.calc(exps[0], server), sy.calc(exps[1], server));
		} else if (par1.contains("!=")) {
			exps = par1.split("!=");
			this.condition = new NotEqualsExpression(sy.calc(exps[0], server), sy.calc(exps[1], server));
		}
	}
}
