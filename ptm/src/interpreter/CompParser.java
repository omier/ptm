...
public class CompParser implements Parser {
	...
	public CompParser(ArrayList<String[]> lines) {
		...
		commandFactory.insert("openDataServer", OpenDataServer.class);
...
//        commandTable.put("disconnect",new CommandExpression(new DisconnectCommand()));
	...
	private Command parseCondition(ArrayList<String[]> array) {
		...
		ConditionCommand c = (ConditionCommand) commandFactory.getNew(array.get(0)[0]);
		int i = 0;
		ArrayList<CommandExpression> tmp = new ArrayList<>();
		CommandExpression cmdtmp = new CommandExpression((Command) commandFactory.getNew("predicate"));
	...
	private ArrayList<CommandExpression> parseCommands(ArrayList<String[]> array) {
		...
		for (int i = 0; i < array.size(); i++) {
			CommandExpression e = new CommandExpression((Command) commandFactory.getNew(array.get(i)[0]));
			if (e.getC() != null) {
				...
				e = new CommandExpression((Command) commandFactory.getNew(array.get(i)[1]));
				e.setS(array.get(i));
			}
			commands.add(e);
		}
